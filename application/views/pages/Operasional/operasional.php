...
<div class="container-xxl flex-grow-1 container-p-y">
	<h4 class="py-1 mb-3"><span class="text-muted fw-light"></span> operasional</h4>
	<div class="card">
		<div class="row">

		</div>
		<div class="d-flex align-items-center justify-content-between">
			<h5 class="card-header">Halaman Operasional</h5>
			<div class="card-header d-flex flex-row justify-content-end gap-3">
				...
				<a href="<?= base_url('C_operasional/tambah') ?>" class="btn btn-primary">
					<i class="menu-icon tf-icons mdi mdi-plus"></i>
					<div style="font-size:10px;">Tambah operasional</div>
				</a>
						...
						<tr>
							...
							<td class="text-center">
								<button style="padding: 0; border: none; background: none;"><a
										onclick="edit(<?php echo $us['id_operasional']; ?>,'operasional')"
										class="btn btn-sm btn-outline-warning text"
										style="color:#ffc107; font-size:10px;">Non
										Edit</a></button>
								<button style="padding: 0; border: none; background: none;"><a
										onclick="hapus(<?php echo $us['id_operasional']; ?>,'operasional')"
...
<script>
	$(document).ready(function () {
		...
		function load_data(query) {
			$.ajax({
				url: "<?= base_url(); ?>C_Operasional/fetch",
				method: "POST",
				data: {
					query: query
				},
