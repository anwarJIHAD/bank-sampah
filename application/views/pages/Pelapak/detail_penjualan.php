...
<div class="container-xxl flex-grow-1 container-p-y">
	<h4 class="fw-bold py-3 mb-4"> <a class="text-muted fw-light" href="<?= base_url('C_Penjualan') ?>">Penjualan Pelapak/</a>
		Detail</h4>
	<div class="card">
		<div class="row">

		</div>
		<h5 class="card-header">Halaman Detail Penjualan<h6 style="margin-left:20px;">Nama Pelapak :
				<?= $pelapak['nama'] ?>
			</h6>
		</h5>
		<div class="d-flex align-items-center justify-content-between">
			...
			<div class="card-header d-flex flex-row justify-content-end gap-3">
				...
				<a href="<?= base_url('C_Penjualan/tambah/' . $id_pelapak) ?>" class="btn btn-primary">
					<i class="menu-icon tf-icons mdi mdi-plus"></i>
					<div style="font-size:10px;">Tambah Penjualan</div>
				</a>
...
<script>
	$(document).ready(function () {
		...
		function load_data(query) {
			$.ajax({
				url: "<?= base_url(); ?>C_Penjualan/fetch_detail/" + <?= $id_pelapak ?>,
				method: "POST",
				data: {
					query: query
				},
